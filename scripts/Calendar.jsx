...
export default function Calendar() {
  ...
  const century = parseFloat(currentYear.toString().slice(0, 2));
  const year = parseFloat(currentYear.toString().slice(-2));
  const calMonth = currentTime.toLocaleString('default', { month: 'long' });
  const firstPart = currentDate + parseFloat((2.6 * currentMonth - 0.2).toString().slice(0, 4));
  const secondPart = 2.0 * parseFloat(century);
  const thirdPart = parseFloat((firstPart - secondPart).toString().slice(0, 4));
  const fourthPart = thirdPart + year + year / 4 + century / 4;
  const fifthPart = Math.trunc(fourthPart % 7);
  let i = 0;
  let x = 1;
